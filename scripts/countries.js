...
function createCountryList() {
    const countriesNum = countryList.length;

    let i = 0,
        ul_id;

    countryList.forEach((country, index) => {
        if (index % Math.ceil(countriesNum / listNum) == 0) {
            ul_id = `list-${i}`;
            countryContainer.innerHTML += `<ul id='${ul_id}'></ul>`;
            i++;
        }
...
// Reseting country list (Showing all the countries)
